...
export default function AuthModal({ isOpen, onClose, onSuccess }: AuthModalProps) {
  ...
  return (
    <div className="fixed inset-0 bg-black/60 backdrop-blur-sm z-50 flex items-center justify-center p-4">
      <div className="bg-gray-100 rounded-3xl max-w-md w-full p-8 shadow-2xl relative">
        {/* Close Button */}
        <button
          onClick={onClose}
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-600 text-sm">
            {mode === 'signin' && 'Continue de onde parou'}
            {mode === 'signup' && 'Crie uma conta para salvar suas análises'}
        ...
        {/* Messages */}
        {error && (
          <div className="mb-4 p-4 rounded-2xl bg-red-900 border border-red-500 text-red-300 text-sm">
            {error}
          </div>
        )}
        {message && (
          <div className="mb-4 p-4 rounded-2xl bg-green-800 border border-green-500 text-green-300 text-sm">
            {message}
          </div>
        )}
